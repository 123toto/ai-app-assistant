...
import { AiDocsSettingsController, createAiDocsSettingsClient } from "./settings.js";

describe("createAiDocsSettingsClient", () => {
  it("calls the portable administration contract", async () => {
    ...
    const client = createAiDocsSettingsClient({ endpoint: "/api/ai-docs/", fetch: fetch as typeof globalThis.fetch });
    ...
    expect(fetch.mock.calls[0]?.[0]).toBe("/api/ai-docs/providers");
    expect(fetch.mock.calls[1]?.[0]).toBe("/api/ai-docs/access");
    expect(fetch.mock.calls[2]?.[0]).toBe("/api/ai-docs/configuration/test");
...
describe("AiDocsSettingsController", () => {
  it("loads optional directory values without making them mandatory", async () => {
    const controller = new AiDocsSettingsController({
